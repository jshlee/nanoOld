...
private:
  ...
  edm::EDGetTokenT<SimTrackContainer> simTrackLabel_;
  edm::EDGetTokenT<SimVertexContainer> simVertexLabel_;
...
longDecayProducer::longDecayProducer(const edm::ParameterSet & iConfig) :
  ...
  simTrackLabel_(consumes<SimTrackContainer>(iConfig.getParameter<edm::InputTag>("simTrackLabel"))),
  simVertexLabel_(consumes<SimVertexContainer>(iConfig.getParameter<edm::InputTag>("simVertexLabel"))),
...
longDecayProducer::produce( edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (auto const& gen_vertex : *trackingVertexs.product()) {
    ...
    for (TrackingVertex::tp_iterator source = gen_vertex.sourceTracks_begin();
      ...
      auto tvx = *source;
      cout << " # tvx = " << " pdg = " << tvx->pdgId() << ", pt = " << tvx->p4().Pt() << endl;
      for (unsigned int i = 0; i < gen_vertex.nDaughterTracks(); ++i){
	auto dau = gen_vertex.daughterTracks().at(i);
  ...
  iEvent.getByToken(simTrackLabel_,simTracks);

  cout <<"simTracks->size() "<< simTracks->size()<<endl;
  // for(size_t i=0, end=simTracks->size(); i<end; ++i) {
  //   auto tpRef = &(*simTracks.product())[i];
  //   if (tpRef->type() != 310) continue;    
  //   cout << " # s = " << i<< " pdg = " << tpRef->type() << ", pt = " << tpRef->momentum().Pt() << endl;
  // }

  Handle<SimVertexContainer> simVertexs;
  iEvent.getByToken(simVertexLabel_,simVertexs);

  cout <<"simVertexs->size() "<< simVertexs->size()<<endl;
  // for(size_t i=0, end=simVertexs->size(); i<end; ++i) {
  //   auto tpRef = &(*simVertexs.product())[i];
  //   cout << " # v = " << i<< " pdg = " << tpRef->type() << ", pt = " << tpRef->position().pt()
  // 	 << ", r = " << tpRef->position().R()
  // 	 << endl;
  // }
...
reco::VertexCompositeCandidate longDecayProducer::fit(vector<const reco::Candidate*>& cands,
						   reco::Vertex& pv, int pdgId,
						   float &dca, float &angleXY, float &angleXYZ)
{
  int charge = 0;
...
longDecayProducer::trackVars longDecayProducer::getTrackVars(vector<const reco::Candidate*>& cands,
						       reco::Vertex& pv)
{
...
longDecayProducer::SVector3
longDecayProducer::getDistanceVector(int dim, reco::VertexCompositeCandidate& vertex,reco::Vertex& pv)
{
  float z = 0.;
  if (dim == 3){
    z = vertex.vz() - pv.position().z();
  }
...
pair<float, float> longDecayProducer::getDistance(int dim, reco::VertexCompositeCandidate& vertex,reco::Vertex& pv)
{
...
int longDecayProducer::findMiniMCMatch(const pat::Jet & aPatJet,
				    reco::VertexCompositeCandidate& candD0,
				    vector<const reco::Candidate*>& cands_D0,
				    Handle<edm::View<pat::PackedGenParticle>>& packed,
...
int longDecayProducer::findMCmatch(const pat::Jet & aPatJet, vector<const reco::Candidate*>& cands, int pdgid)
{
...
bool longDecayProducer::isAncestor(const reco::Candidate* ancestor, const reco::Candidate * particle)
{
