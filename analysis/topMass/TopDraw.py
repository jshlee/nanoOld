...
datasets = json.load(open("/cms/scratch/jdj0715/nanoAOD/src/nano/analysis/data/dataset/dataset.json"))
#cut_step = "(step>=5)"
...
#plotvar = 'njet'
plotvar = 'Dilep.M()'
binning = [60, 20, 320]#[150, 50, 200]
#binning = [10, 0, 10]#[150, 50, 200]
#x_name = 'Invariant Mass[GeV]'
x_name = 'mass(\mu^+\mu^-)(GeV)'
y_name = 'Events'
dolog = True
f_name = 'Dilep'
#minp = 0.05
...
"""
    ...
    lumi3 = 0
    while ((sig[0]/math.sqrt(sig[0]+bg[0]))<3):
        rescale = lumi3*data["xsec"]/wentries
        remchist = makeTH1(rfname, tname, title, binning, plotvar, tcut, rescale)    
        if "ttH" in mcname:
            sig[0]+= remchist.Integral(remchist.FindBin(120),remchist.FindBin(130))
        else:
            bg[0]+= remchist.Integral(remchist.FindBin(120),remchist.FindBin(130))
        if (imc < len(mcfilelist)-1):break
        lumi3 += 10
    lumilist.append(lumi3)
    lumi5 = 0
    while ((sig[l]/math.sqrt(sig[l]+bg[l]))<5):
        lumi5 += 10
