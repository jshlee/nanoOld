...
'''
topDraw.py -c 'll_m>50&&step>=5&&isTight==1&&filtered==1' -b [40,0,40] -p nvertex -x 'no. vertex' &
topDraw.py -c 'll_m>50&&step>=5&&isTight==1&&filtered==1' -b [200,0,200] -p ll_m -x 'mass [GeV]' &
topDraw.py -c 'll_m>50&&step>=5&&isTight==1&&filtered==1' -b [100,0,100] -p ll_pt -x 'diMuon p_{T}  [GeV]' &
topDraw.py -c 'll_m>50&&step>=5&&isTight==1&&filtered==1' -b [100,0,100] -p lep1_pt -x 'leading muon p_{T}  [GeV]' &
topDraw.py -c 'll_m>50&&step>=5&&isTight==1&&filtered==1' -b [100,0,100] -p lep2_pt -x 'sub-leading muon p_{T}  [GeV]' &
topDraw.py -c 'll_m>50&&step>=5&&isTight==1&&filtered==1' -b [100,0,100] -p lep1_pt,lep2_pt -x 'muon p_{T}  [GeV]' &
topDraw.py -c 'll_m>50&&step>=5&&isTight==1&&filtered==1' -b [100,0,100] -p met -x 'met  [GeV]' &
topDraw.py -c 'll_m>50&&step>=5&&isTight==1&&filtered==1' -b [100,-3,3] -p lep1_eta,lep2_eta -x '#eta' &
'''

json_used = 'Golden'
#datalumi =  5975#35.9fb-1
#datalumi =  8360 #35.9fb-1
#datalumi = 8360
datalumi = 36814
#datalumi = 8360.481454 #35.9fb-1
version = os.environ['CMSSW_VERSION']

#rootfileDir = "%s/src/nano/analysis/h2mu/Results/Nano_C_Test/results_merged/ttbar_"% os.environ['CMSSW_BASE']
rootfileDir = "/cms/scratch/jdj0715/nanoAOD3/src/nano/analysis/h2mu/Results/Nano_C_Test/results_merged/ttbar_"
...
CMS_lumi.lumi_sqrtS = "%.2f fb^{-1}, #sqrt{s} = 13 TeV "%(float(datalumi)/1000)
mcfilelist = [
#              'WMinusH_HToMuMu',
#              'WPlusH_HToMuMu',
#              'ZH_HToMuMu',
...
datasets = json.load(open("/cms/scratch/jdj0715/nanoAOD3/src/nano/analysis/data/dataset/dataset.json"))
#cut_step = "(step>=5)"
#cut = 'lep1.Pt()>60&&lep2.Pt()>60&&dilep.M()>60&&step>=5'
#cut = 'dilep.M()>60&&step>4&&filtered&&MVA_BDT>-0.0246'
cut = 'Step>=1'#Dilep.M()>60'
...
weight = 'genweight'
#plotvar = 'met'
plotvar = 'Dilep.M()'#'Nu_Jet'#'Nu_BJet'#'Dilep.M()'
binning = [60, 20, 320]#[6,0,6]#[60, 20, 320]#[150, 50, 200]#[150, 50, 200]
x_name = 'Invariant Mass[GeV]'
#x_name = 'number_of_jets'
y_name = 'Events'
dolog = True
...
lumilist = [datalumi, 831.76, 18610, 6025.2]
#if plotvar == 'Dilep.M()':
